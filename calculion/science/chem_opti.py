...
class SteadyStateOpti(object):
    ...
    def _ss_gnopti_funk(self,
        ...
        '''
        Gauss Newton optimization function computed at each iteration of the solver.
        ...
        const_params: Union[list, ndarray]
            Values of the constants, ordered according to self.args_list.

        '''
        # Set parameter vals to the mother object:
        self._bes._set_param_vals(self.ss_params_list, ss_params, self.keep_positive)
        self._bes._set_param_vals(self.args_list, const_params)

        # Get a list of param values in the correct order:
        divflux_param_vals = self._bes._get_param_vals(self._bes.divflux_params_list)

        # Calculate flux residuals:
        resids = self._bes.divflux_f(*divflux_param_vals)

        # Calculate the jacobian
        jac = self.jac_f(ss_params, const_params)
    ...
    def gauss_newton(self, opti_N: int=1000, tol: float=1.0e-15):
        ...
        '''
        param_vals = self.param_o # Initialize the parameter values
        sum_sq = 99999.9 # Initialize sum of squares

        for ii in range(opti_N):
            # Calculate vector of residuals and Jacobian matrix:
            rri, jji = self._ss_gnopti_funk(param_vals, self.args_vals)

            #Update the parameter values:
            param_vals += -((jji.dot(rri.T)).T)[0]

            for i, (pi, posi) in enumerate(zip(param_vals, self.keep_positive)):
                if pi < 0.0 and posi is True:
                    pi = 0.0
                    param_vals[i] = pi
            ...
            # If sum of squares less than tolerance
            if sum_sq < tol:
                print(f'Tolerance reached, stopping at iteration {ii}, sum square error {sum_sq}')
                break

        return param_vals, sum_sq

    def scipy_opti(self, method='L-BFGS-B', bounds=None):
        '''

        '''
        self.sol0 = minimize(self.divflux_opti_f,
                        self.param_o,
                        args=(self.args_vals),
                        jac=self.jac_opti_f,
                        # hess=self.hess_opti_f,
                        method=method,
                        bounds=bounds)

        return self.sol0.x, self.sol0.fun

    def basinhop_opti(self,
                      method: str='L-BFGS-B',
                      bounds: Optional[list]=None,
                      disp: bool=False):
        '''
        Search for a global minimum of the optimization function using Scipy's Basinhopping
        algorithm.

        Parameters
        ----------
        method : str
            Method to use with Scipy's minimization algorithm (must work with bounds)
        bounds : None
            Bounds on parameters as list of (min, max) values for each parameter
        disp: bool
            Display feedback messages while solving (True)?
        '''
        min_kwargs = {}
        min_kwargs['method'] = method
        min_kwargs['args'] = self.args_vals
        min_kwargs['bounds'] = bounds
        min_kwargs['jac'] = self.jac_opti_f
        min_kwargs['hess'] = self.hess_opti_f
        self.sol1 = basinhopping(self.divflux_opti_f,
                            self.param_o,
...
class FluxSim(object):
    ...
    academic differences such as forward, reverse, and symmetric flux expressions.

    Public Attributes
    -----------------

    flux_time : ndarray
        Average reaction flux at each time step [mol/(m^2 s)].

    flux_f_time : ndarray
        Forward reaction flux at each time step [mol/(m^2 s)].

    flux_r_time : ndarray
        Reverse reaction flux at each time step [mol/(m^2 s)].

    concs_time : ndarray
        Concentrations as a function of time, calculated using average flux [mol/m^3].

    concs_f_time : ndarray
        Concentrations as a function of time, calculated using forward flux [mol/m^3].

    concs_r_time : ndarray
        Concentrations as a function of time, calculated using reverse flux [mol/m^3].

    time : ndarray
        Time value at each step of the iteration [s].

    flux_f_funk : function
        Numerical forward flux computing function.

    flux_f_param_names : list
        Symbolic parameter names for forward flux computing function.

    flux_f_param_str : list
        String parameter names for forward flux computing function.

    flux_funk : function
        Numerical average flux computing function.

    flux_param_names : list
        Symbolic parameter names for average flux computing function.

    flux_param_str : list
        String parameter names for average flux computing function.

    flux_r_funk : function
        Numerical reverse flux computing function.

    flux_r_param_names : list
        Symbolic parameter names for reverse flux computing function.

    flux_r_param_str : list
        String parameter names for reverse flux computing function.

    prod_f_indices : list
        Integer indices to products in forward flux reaction params.

    prod_indices : list
        Integer indices to products average flux reaction params.

    prod_r_indices : list
        Integer indices to products in reverse flux reaction params.

    react_f_indices : list
        Integer indices to reactants in forward flux reaction params.

    react_indices : list
        Integer indices to reactants in average flux reaction params.

    react_r_indices : list
        Integer indices to reactants in reverse flux reaction params.
    ...
    def __init__(self, reaction: ReactionABC):
        ...
        reaction : ReactionABC
            A reaction to simulate.

        '''

        self.flux_funk, self.flux_param_names = reaction.get_numerical(reaction.react_flx_s)
        self.flux_f_funk, self.flux_f_param_names = reaction.get_numerical(reaction.react_flx_f_s)
        self.flux_r_funk, self.flux_r_param_names = reaction.get_numerical(reaction.react_flx_r_s)

        # Convert to string names:
        self.flux_param_str = []
        for pn in self.flux_param_names:
            self.flux_param_str.append(pn.name)

        self.flux_r_param_str = []
        for pn in self.flux_r_param_names:
            self.flux_r_param_str.append(pn.name)

        self.flux_f_param_str = []
        for pn in self.flux_f_param_names:
            self.flux_f_param_str.append(pn.name)

        # Get the indices for concentration updates:
        self.react_indices = []
        self.react_f_indices = []
        self.react_r_indices = []
        for ri in reaction._react_list:
            irea = self.flux_param_str.index(ri.name)
            ireaf = self.flux_f_param_str.index(ri.name)
            irear = self.flux_r_param_str.index(ri.name)
            self.react_indices.append(irea)
            self.react_f_indices.append(ireaf)
            self.react_r_indices.append(irear)

        self.prod_indices = []
        self.prod_f_indices = []
        self.prod_r_indices = []
        for pi in reaction._prod_list:
            ipro = self.flux_param_str.index(pi.name)
            iprof = self.flux_f_param_str.index(pi.name)
            ipror = self.flux_r_param_str.index(pi.name)
            self.prod_indices.append(ipro)
    ...
    def generate_params(self, p: BioeParams):
        ...
        # Scrape param values from p:
        self.flux_funk_params = []
        for pn in self.flux_param_names:
            pv = getattr(p, pn.name, None)
            if pv is not None:
                self.flux_funk_params.append(pv)
            else:
                raise Exception(f'Parameter {pn.name} not found in p!')

        self.flux_f_funk_params = []
        for pn in self.flux_f_param_names:
            pv = getattr(p, pn.name, None)
            if pv is not None:
                self.flux_f_funk_params.append(pv)
            else:
                raise Exception(f'Parameter {pn.name} not found in p!')

        self.flux_r_funk_params = []
        for pn in self.flux_r_param_names:
            pv = getattr(p, pn.name, None)
            if pv is not None:
                self.flux_r_funk_params.append(pv)
            else:
                raise Exception(f'Parameter {pn.name} not found in p!')
    ...
    def sim(self, Niter: int, dt: float):
        ...
        # Perform the iterative simulation:
        ...
        for i in range(Niter):
            ...
            react_flx = self.flux_funk(*self.flux_funk_params)
            react_f_flx = self.flux_f_funk(*self.flux_f_funk_params)
            react_r_flx = -self.flux_r_funk(*self.flux_r_funk_params)

            ct = []
            for irea in self.react_indices:
                self.flux_funk_params[irea] += react_flx * dt
                ct.append(self.flux_funk_params[irea]*1)

            for iprod in self.prod_indices:
                self.flux_funk_params[iprod] += -react_flx*dt
                ct.append(self.flux_funk_params[iprod] * 1)

            ctf = []
            for irea in self.react_f_indices:
                self.flux_f_funk_params[irea] += react_f_flx * dt
                ctf.append(self.flux_f_funk_params[irea] * 1)

            for iprod in self.prod_f_indices:
                self.flux_f_funk_params[iprod] += -react_f_flx*dt
                ctf.append(self.flux_f_funk_params[iprod] * 1)

            ctr = []
            for irea in self.react_r_indices:
                self.flux_r_funk_params[irea] += react_r_flx * dt
                ctr.append(self.flux_r_funk_params[irea] * 1)

            for iprod in self.prod_r_indices:
                self.flux_r_funk_params[iprod] += -react_r_flx*dt
                ctr.append(self.flux_r_funk_params[iprod] * 1)

            time.append(tt)

            flux_time.append(react_flx * 1)
            flux_f_time.append(react_f_flx * 1)
            flux_r_time.append(react_r_flx * 1)

            concs_time.append(ct)
            concs_f_time.append(ctf)
            concs_r_time.append(ctr)

        self.flux_time = np.asarray(flux_time)
        self.flux_f_time = np.asarray(flux_f_time)
        self.flux_r_time = np.asarray(flux_r_time)

        self.concs_time = np.asarray(concs_time)
        self.concs_f_time = np.asarray(concs_f_time)
        self.concs_r_time = np.asarray(concs_r_time)
